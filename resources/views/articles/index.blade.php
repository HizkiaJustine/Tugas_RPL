                    ...
                    <div class="col-md-4">
                        <div class="card mb-4">
                            <div class="card-body">
                                ...
                                <p class="card-text">{{ Str::limit($article->content, 100) }}</p> <!-- Limit content to 100 characters -->
                                <p class="card-text"><small class="text-muted">Published on: {{ $article->publishDate }}</small></p>
                                <a href="{{ route('articles.show', $article->articleId) }}" class="btn btn-primary">Read More</a>
                            </div>
                        </div>
                    </div>
                    @if (!$loop->last)
                        <div class="w-100 my-4" style="border-top: 1px solid #ccc;"></div> <!-- Visible Separator -->
                    @endif
                @endforeach
            </div>
        </div>
